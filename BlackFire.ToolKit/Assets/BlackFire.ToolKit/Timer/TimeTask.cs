...
namespace BlackFire.ToolKit
{
    ...
    public sealed class TimeTask
    {
        ...
        internal void Update()
        {
            ...
            {
                case TimeTaskType.Delay:
                    if (0f >= (m_DelayTime -= Time.deltaTime)) { Emit(); OnRecycle(); };
                    break;
                case TimeTaskType.DelayFrame:
                    if (0 >= (m_DelayFrame--)) { Emit(); OnRecycle(); };
                    break;
                case TimeTaskType.Loop:
                    if (0f < (m_LoopTime -= Time.deltaTime)) { Emit(); } else { OnRecycle(); };
                    break;
                case TimeTaskType.LoopFrame:
                    if (0 < (m_LoopFrame--)) { Emit(); } else { OnRecycle(); };
                    break;
                case TimeTaskType.Interval:
                    if (m_IntervalTime <= (m_TempIntervalTime += Time.deltaTime)) { m_TempIntervalTime = 0f; Emit(); }
                    break;
                case TimeTaskType.IntervalFrame:
                    if (0 == (m_TempIntervalFrame--) % m_IntervalFrame) Emit();
                    break;
                case TimeTaskType.RealDelay:
                    if (0f >= (m_RealDelayTime -= Time.unscaledDeltaTime)) { Emit(); OnRecycle(); };
                    break;
                case TimeTaskType.RealLoop:
                    if (0f < (m_RealLoopTime -= Time.unscaledDeltaTime)) { Emit(); } else { OnRecycle(); };
                    break;
                case TimeTaskType.RealInterval:
                    if (m_RealIntervalTime <= (m_TempRealIntervalTime += Time.unscaledDeltaTime)) { m_TempRealIntervalTime = 0f; Emit(); }
                    break;
                default:
                    break;
            }
